...
*Setup instructions for a Gitian build of Maza Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Maza
...
- [Building Maza Core](#building-maza-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-maza-host/README.md).
...
Clone the git repositories for maza and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mazacoin/maza
git clone https://github.com/maza-core/gitian.sigs.git
```
...
in the maza repository under 'Fetch and create inputs' to install sources which require
...
To build Maza Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the maza repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/maza/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mazacoin/maza
...
For example:
```bash
URL=https://github.com/laanwj/maza.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit maza=${COMMIT} --url maza=${URL} ../maza/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit maza=${COMMIT} --url maza=${URL} ../maza/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit maza=${COMMIT} --url maza=${URL} ../maza/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the maza git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../maza/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/maza
SIGPATH=/some/root/path/maza-detached-sigs

./bin/gbuild --url maza=${BTCPATH},signature=${SIGPATH} ../maza/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/maza-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/maza-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/maza-osx-build.assert
```
...
[maza-core/gitian.sigs](https://github.com/maza-core/gitian.sigs/) repository, or if that's not possible create a pull
